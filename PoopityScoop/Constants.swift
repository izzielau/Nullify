//
//  File.swift
//  PoopityScoop
//
//  Created by Isabella Lau on 11/2/18.
//  Copyright © 2018 Isabella Lau. All rights reserved.
//

import UIKit

class Constants {
    static let LIGHT_BLUE = UIColor(red: 160.0/255.0, green: 203.0/255.0, blue: 255.0/255.0, alpha: 1.0)
    static let BLUE = UIColor(red: 117.0/255.0, green: 179.0/255.0, blue: 255.0/255.0, alpha: 1.0)
    static let DARK_BLUE = UIColor(red: 109.0/255.0, green: 158.0/255.0, blue: 235.0/255.0, alpha: 1.0)
    static let DARKER_BLUE = UIColor(red: 109.0/255.0, green: 158.0/255.0, blue: 255.0/255.0, alpha: 1.0)
    
    static let DARK_GRAY = UIColor(red: 30.0/255.0, green: 30.0/255.0, blue: 30.0/255.0, alpha: 1.0)
    
    static let LIGHT_MINT = UIColor(red: 59.0/255.0, green: 194.0/255.0, blue: 153.0/255.0, alpha: 1.0)
    static let DARK_MINT = UIColor(red: 31.0/255.0, green: 140.0/255.0, blue: 115.0/255.0, alpha: 1.0)
    
    static let LIGHT_CORAL = UIColor(red: 247.0/255.0, green: 118.0/255.0, blue: 127.0/255.0, alpha: 1.0)
    static let DARK_CORAL = UIColor(red: 184.0/255.0, green: 74.0/255.0, blue: 89.0/255.0, alpha: 1.0)
}
